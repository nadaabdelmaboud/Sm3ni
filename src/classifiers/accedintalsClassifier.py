...
def extractAccedintalsFeatures(img):
    img = 1 - img
...
def accedintalsClassifier(test_img_path):
    path_to_dataset = r'acccedintals_dataset'
    random_seed = 42
    ...
    nn = classifiers['KNN']
    filename = 'accedintals_model.sav'
    pickle.dump(nn, open(filename, 'wb'))

def getAccedintals(result):
    if(result == 'a'):
        accedintal = "#"
    if(result == 'b'):
        accedintal = "&"
